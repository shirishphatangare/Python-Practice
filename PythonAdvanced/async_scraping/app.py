...
from async_scraping.pages.all_books_page import AllBooksPage
...
async def fetch_page(session, url):
    ...
    async with session.get(url) as response: # async block
        print(f'{url} took {time.time() - page_start}')
        return await response.text() # wait for response


async def get_multiple_pages(loop, *urls):
    tasks = []
    async with aiohttp.ClientSession(loop=loop) as session: # Single session object for optimization
        for url in urls:
            tasks.append(fetch_page(session, url))
        return await asyncio.gather(*tasks) # wait for all tasks to complete and
...
# http://google.com
urls = [f'http://books.toscrape.com/catalogue/page-{page_num+1}.html' for page_num in range(page.page_count)] # website books.toscrape.com do not allow simultaneous requests. That is why response time increases with each subsequent request
start = time.time()
...
for page_content in pages:
    logger.info('Creating AllBooksPage from page content.')
    page = AllBooksPage(page_content)
